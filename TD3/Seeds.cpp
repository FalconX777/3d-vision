                    ...
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    ...
    for (int x=-win; x <= win; x++){
        for (int y=-win; y <= win; y++){
            e_x2 += pow(im1(i1+x,j1+y)-m1,2);
            e_y2 += pow(im2(i2+x,j2+y)-m2,2);
            e_xy += (im1(i1+x,j1+y)-m1)*(im2(i2+x,j2+y)-m2);
        }
    }
    dist = e_xy/(sqrt(e_x2)*sqrt(e_y2));
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            for (int d=dMin; d<=dMax; d++){
                ...
                float currCcorrel = ccorrel(im1, x, y, im2, x+d, y );
                if(currCcorrel>nccMax){
                    bestd = d;
                    nccMax = currCcorrel;
                }
            }
            if(nccMax>nccSeed){
                seeds(x,y) = true;
                disp(x,y) = bestd;
                Seed newSeed = Seed(x, y, bestd, nccSeed);
                Q.push(newSeed);
            }
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                ...
                for(int d=1; d>=-1; d--){
                    ...
                    float currCcorrel = ccorrel(im1, x, y, im2, x + totald, y );
                    if(currCcorrel>bestCcorrel){
                        bestd = totald;
                        bestCcorrel = currCcorrel;
                    }
                }
                if(bestd<dMin){
                    bestd = dMin;
                ...
                seeds(x,y) = true;
                disp(x,y) = bestd;
                Seed newSeed = Seed(x, y, bestd, bestCcorrel);
                Q.push(newSeed);
            }
        }
    }
